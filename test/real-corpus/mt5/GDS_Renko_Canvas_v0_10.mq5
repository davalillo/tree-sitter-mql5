...
#property description "Classic fixed-size Renko rendered with one MQL5 Canvas layer."
#property description "Square completed bricks are built from tick data in real time."
#property description "No DLLs, custom symbols or offline charts are required."
#property indicator_chart_window
#property indicator_plots 0

#include <Canvas\Canvas.mqh>

//--- Intentionally simple CodeBase input.
...
bool EnsureCanvas(void)
  {
   ...
   if(!g_canvas.CreateBitmapLabel(0,0,g_canvas_name,0,0,wanted_width,wanted_height,COLOR_FORMAT_XRGB_NOALPHA))
     {
      Print("GDS Renko Canvas: could not create canvas. Error ",GetLastError());
      return false;
     }
...
void DrawLoading(void)
  {
   ...
   g_canvas.TextOut(12,12,"RENKO CANVAS  |  Loading tick history...",ColorToARGB(GDS_TEXT_COLOR,255));
...
void RenderCanvas(void)
  {
   ...
   if(!g_history_ready || total<=0)
     {
      const string status=(g_history_ready ? "RENKO CANVAS  |  Waiting for first brick..." :
                                             "RENKO CANVAS  |  Loading tick history...");
   ...
   if(visible<=0)
     {
      g_canvas.TextOut(12,12,"RENKO CANVAS  |  Chart window is too small",ColorToARGB(GDS_TEXT_COLOR,255));
      g_canvas.Update(true);
      return;
     }
   ...
   const double fixed_min=min_price-GDS_PRICE_PADDING_BRICKS*InpBrickSize;
   const double fixed_max=fixed_min+full_price_span;
   ...
   const string header="RENKO CANVAS  |  Brick: "+DoubleToString(InpBrickSize,_Digits);
   ...
   if(count<=0)
     {
      ...
      if(g_history_attempts==1 || (g_history_attempts%10)==0)
         Print("GDS Renko Canvas: waiting for tick history. Error ",GetLastError());
      return false;
     }
...
int OnInit(void)
  {
   if(InpBrickSize<=0.0)
     {
      Print("GDS Renko Canvas: Brick Size must be greater than zero.");
      return INIT_PARAMETERS_INCORRECT;
     }

   g_canvas_name="GDS_RC_CANVAS_"+IntegerToString((int)ChartID());

   if(SaveChartStyle())
      PrepareRenkoChart();
   else
      Print("GDS Renko Canvas: could not save all chart properties.");
   ...
   IndicatorSetString(INDICATOR_SHORTNAME,
                      "GDS Renko Canvas ("+DoubleToString(InpBrickSize,_Digits)+")");
