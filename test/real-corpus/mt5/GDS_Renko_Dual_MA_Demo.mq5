//+------------------------------------------------------------------+
//|                 GDS Renko Dual MA Demo EA                        |
//|       Educational Renko + two moving averages example            |
//+------------------------------------------------------------------+
#property copyright "Golden Delta"
#property link      "https://goldendeltaea.com/"
#property version   "1.10"
#property strict
#property description "Educational Renko + Dual MA Expert Advisor for MetaTrader 5."
#property description "Builds classic fixed-size Renko internally from BID ticks."
#property description "Fast and slow simple moving averages use completed Renko closes only."
#property description "A completed brick may trigger a trade only in the current MA direction."
...
input int    InpFastMAPeriod           = 9;     // Fast SMA period, completed Renko bricks
input int    InpSlowMAPeriod           = 152;    // Slow SMA period, completed Renko bricks
input double InpMinMASeparationBricks  = 4.15;  // Minimum Fast/Slow separation in brick units
input int    InpEntryRunBricks         = 2;     // Minimum same-direction completed brick run
input double InpTakeProfitBricks       = 55.0;   // Virtual take profit in bricks
input double InpStopLossBricks         = 32.0;   // Virtual stop loss in bricks
input int    InpMaxHoldMinutes         = 1440;   // Maximum hold; 0 disables time exit
input int    InpCooldownBricks         = 16;     // Completed bricks to wait after exit
input double InpMaxSpreadFraction      = 0.35;  // Maximum spread / brick size
input double InpLots                   = 0.01;  // Requested fixed lot
input ulong  InpMagic                  = 26091051; // Unique EA magic; do not optimize
...
//+------------------------------------------------------------------+
//| Dual SMA calculated only from completed Renko closes             |
//+------------------------------------------------------------------+
class CRenkoDualMA
  {
private:
   double m_closes[];
   int    m_fast,m_slow,m_count;
   double m_brick,m_min_sep;
   double m_fast_value,m_slow_value;
   bool   m_ready;
public:
   bool Init(const int fast_period,const int slow_period,
             const double brick_size,const double min_sep_bricks)
     {
      m_fast=fast_period;
      m_slow=slow_period;
      m_count=0;
      m_brick=brick_size;
      m_min_sep=min_sep_bricks;
      m_fast_value=0.0;
      m_slow_value=0.0;
      m_ready=false;
   ...
   int Push(const SRenkoBrick &brick)
     {
      if(m_count<m_slow)
        {
         m_closes[m_count]=brick.close;
         m_count++;
        }
      else
        {
         for(int i=1;i<m_slow;i++)
            m_closes[i-1]=m_closes[i];
         m_closes[m_slow-1]=brick.close;
        }
      ...
      double slow_sum=0.0;
      double fast_sum=0.0;
      const int fast_start=m_slow-m_fast;
      for(int i=0;i<m_slow;i++)
        {
         slow_sum+=m_closes[i];
         if(i>=fast_start) fast_sum+=m_closes[i];
        }

      m_fast_value=fast_sum/(double)m_fast;
      m_slow_value=slow_sum/(double)m_slow;
      m_ready=true;

      const double separation=m_min_sep*m_brick;
      const double eps=1e-9;
      if(m_fast_value>m_slow_value+separation-eps) return 1;
      if(m_fast_value<m_slow_value-separation+eps) return -1;
      return 0;
     }

   bool Ready(void) { return m_ready; }
   double FastValue(void) { return m_fast_value; }
   double SlowValue(void) { return m_slow_value; }
  };

CRenkoBuilder g_renko;
CRenkoDualMA  g_dualma;
...
void RegisterFailure(const uint retcode,const string details)
  {
   ...
   Print("GDS Renko Dual MA Demo: request deferred. Retcode ",retcode,
                ...
                result.retcode!=TRADE_RETCODE_PLACED))
     {
      if(result.retcode==TRADE_RETCODE_TIMEOUT ||
         result.retcode==TRADE_RETCODE_CONNECTION || result.retcode==0)
        {
         ...
         Print("GDS Renko Dual MA Demo: execution unconfirmed; requests paused.");
        }
...
bool SendMarket(const int direction)
  {
   ...
   request.comment="GDS Renko Dual MA";
...
bool CloseOurPosition(const string reason)
  {
   ...
   request.deviation=50;
   request.comment="GDS Dual MA exit";
   ...
   if(!FindOurPosition(ticket,type,volume,open_price,open_time))
     {
      Print("GDS Renko Dual MA Demo exit: ",reason);
      MarkExitCompleted();
     }
   return true;
  }
...
//+------------------------------------------------------------------+
//| Only a completed Renko brick can update the MAs or signal.       |
//+------------------------------------------------------------------+
int ProcessCompletedBrick(const SRenkoBrick &brick,int &alignment)
  {
   alignment=g_dualma.Push(brick);
   if(alignment==0) return 0;
   if(brick.direction!=alignment) return 0;
   ...
   if(n<0)
     {
      ...
      Print("GDS Dual MA: excessive gap/memory failure. New entries paused; existing exits remain active.");
      return;
     }
   if(n==0) return;

   int final_signal=0;
   int final_alignment=0;
   for(int i=0;i<n;i++)
     {
      int alignment=0;
      final_signal=ProcessCompletedBrick(bricks[i],alignment);
   ...
   if(have_position)
     {
      ...
      if(final_alignment!=0 && final_alignment==-position_direction)
         RequestExit("MA_FLIP");
      return;
     }
...
int OnInit(void)
  {
   if(!MathIsValidNumber(InpBrickSize) || InpBrickSize<=0.0 ||
      InpFastMAPeriod<2 || InpFastMAPeriod>100 ||
      InpSlowMAPeriod<3 || InpSlowMAPeriod>250 ||
      InpFastMAPeriod>=InpSlowMAPeriod ||
      !MathIsValidNumber(InpMinMASeparationBricks) || InpMinMASeparationBricks<0.0 ||
     ...
     {
      Print("GDS Renko Dual MA Demo: invalid inputs.");
   ...
   if(!MathIsValidNumber(tick_size) || tick_size<=0.0)
     {
      Print("GDS Dual MA: symbol tick size unavailable.");
      return INIT_FAILED;
     }
   ...
   if(!MathIsValidNumber(units_raw) || units_raw<0.5 || units_raw>1e9)
     {
      Print("GDS Dual MA: brick size cannot be represented for this symbol.");
      return INIT_FAILED;
     }
   ...
   if(effective_volume<=0.0)
     {
      Print("GDS Dual MA: symbol volume settings unavailable or invalid.");
      return INIT_FAILED;
     }

   g_renko.Init(tick_size,units);
   if(!g_dualma.Init(InpFastMAPeriod,InpSlowMAPeriod,
                     g_effective_brick_size,InpMinMASeparationBricks))
      return INIT_FAILED;
   ...
   Print("GDS Renko Dual MA Demo v1.10 started. RequestedBrick=",
         DoubleToString(InpBrickSize,_Digits),
         ", EffectiveBrick=",DoubleToString(g_effective_brick_size,_Digits),
         ", FastMA=",InpFastMAPeriod,
         ", SlowMA=",InpSlowMAPeriod,
         ", MinSep=",DoubleToString(InpMinMASeparationBricks,2),
...
void OnTick(void)
  {
   ...
   // MA state and entry decisions only change when BID completes Renko bricks.
   UpdateRenkoAndTrade(tick.bid);

   // A MA flip detected on the completed brick may request an exit now.
   TryPendingExit();
  }
//+------------------------------------------------------------------+
