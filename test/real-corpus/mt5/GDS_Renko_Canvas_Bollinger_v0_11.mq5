//+------------------------------------------------------------------+
//|                                      GDS_Renko_Canvas_Bollinger_v0.11.mq5 |
...
#property description "Classic fixed-size Renko rendered with one MQL5 Canvas layer plus Bollinger Bands."
#property description "Bands are calculated only from completed internal Renko closes."
...
input int    InpBBPeriod    = 20;   // Bollinger period in completed Renko bricks
input double InpBBDeviation = 2.00; // Bollinger deviation multiplier
...
const color GDS_BB_OUTER_COLOR = clrDodgerBlue;
const color GDS_BB_MIDDLE_COLOR= clrGold;
...
bool EnsureCanvas(void)
  {
   ...
   if(!g_canvas.CreateBitmapLabel(0,0,g_canvas_name,0,0,wanted_width,wanted_height,COLOR_FORMAT_XRGB_NOALPHA))
     {
      Print("GDS Renko Canvas + Bollinger: could not create canvas. Error ",GetLastError());
      return false;
     }
...
//+------------------------------------------------------------------+
bool ComputeBollingerAt(const int index,double &middle,double &upper,double &lower)
  {
   middle=0.0;
   upper=0.0;
   lower=0.0;

   if(InpBBPeriod<2 || index<InpBBPeriod-1 || index>=ArraySize(g_bricks))
      return false;

   double sum=0.0;
   for(int k=index-InpBBPeriod+1;k<=index;k++)
      sum+=g_bricks[k].close;

   middle=sum/(double)InpBBPeriod;

   double variance=0.0;
   for(int k=index-InpBBPeriod+1;k<=index;k++)
     {
      const double d=g_bricks[k].close-middle;
      variance+=d*d;
     }
   variance/=(double)InpBBPeriod;

   const double sigma=MathSqrt(MathMax(variance,0.0));
   upper=middle+InpBBDeviation*sigma;
   lower=middle-InpBBDeviation*sigma;
   return true;
  }
   ...
   for(int n=1;n<=max_count;n++)
     {
      ...
      // Keep the visible Bollinger envelope inside the same fixed price scale.
      double bb_mid=0.0,bb_upper=0.0,bb_lower=0.0;
      if(ComputeBollingerAt(i,bb_mid,bb_upper,bb_lower))
        {
         lo=MathMin(lo,bb_lower);
         hi=MathMax(hi,bb_upper);
        }
...
//+------------------------------------------------------------------+
//| Draw a visually stronger outer Bollinger line.                  |
//+------------------------------------------------------------------+
void DrawOuterBandSegment(const int x1,const int y1,const int x2,const int y2)
  {
   const uint c=ColorToARGB(GDS_BB_OUTER_COLOR,255);
...
void DrawLoading(void)
  {
   ...
   g_canvas.TextOut(12,12,"RENKO CANVAS + BOLLINGER  |  Loading tick history...",ColorToARGB(GDS_TEXT_COLOR,255));
...
void RenderCanvas(void)
  {
   ...
   if(!g_history_ready || total<=0)
     {
      const string status=(g_history_ready ? "RENKO CANVAS + BOLLINGER  |  Waiting for first brick..." :
                                             "RENKO CANVAS + BOLLINGER  |  Loading tick history...");
   ...
   if(visible<=0)
     {
      g_canvas.TextOut(12,12,"RENKO CANVAS + BOLLINGER  |  Chart window is too small",ColorToARGB(GDS_TEXT_COLOR,255));
      g_canvas.Update(true);
      return;
     }
   ...
   // Bollinger overlay. Each point is calculated only from completed
   // Renko closes up to that completed brick. The forming brick is never used.
   ...
   for(int i=first;i<total;i++)
     {
      ...
      const int y_upper=PriceToCanvasY(bb_upper,fixed_min,fixed_max);
      const int y_mid=PriceToCanvasY(bb_mid,fixed_min,fixed_max);
      const int y_lower=PriceToCanvasY(bb_lower,fixed_min,fixed_max);

      if(have_prev)
        {
         DrawOuterBandSegment(prev_x,prev_y_upper,x,y_upper);
         DrawOuterBandSegment(prev_x,prev_y_lower,x,y_lower);
         g_canvas.Line(prev_x,prev_y_mid,x,y_mid,ColorToARGB(GDS_BB_MIDDLE_COLOR,255));
        }
   ...
   const string header="RENKO CANVAS + BOLLINGER  |  Brick: "+DoubleToString(InpBrickSize,_Digits)+
                       "  |  BB: "+IntegerToString(InpBBPeriod)+" / "+DoubleToString(InpBBDeviation,2);
   ...
   if(count<=0)
     {
      ...
      if(g_history_attempts==1 || (g_history_attempts%10)==0)
         Print("GDS Renko Canvas + Bollinger: waiting for tick history. Error ",GetLastError());
      return false;
     }
...
int OnInit(void)
  {
   if(InpBrickSize<=0.0)
     {
      Print("GDS Renko Canvas + Bollinger: Brick Size must be greater than zero.");
      return INIT_PARAMETERS_INCORRECT;
     }
   if(InpBBPeriod<2)
     {
      Print("GDS Renko Canvas + Bollinger: Bollinger period must be at least 2.");
      return INIT_PARAMETERS_INCORRECT;
     }
   if(InpBBDeviation<=0.0)
     {
      Print("GDS Renko Canvas + Bollinger: Bollinger deviation must be greater than zero.");
      return INIT_PARAMETERS_INCORRECT;
     }

   g_canvas_name="GDS_RCB_CANVAS_"+IntegerToString((int)ChartID());

   if(SaveChartStyle())
      PrepareRenkoChart();
   else
      Print("GDS Renko Canvas + Bollinger: could not save all chart properties.");
   ...
   IndicatorSetString(INDICATOR_SHORTNAME,
                      "GDS Renko Canvas + Bollinger ("+DoubleToString(InpBrickSize,_Digits)+")");
